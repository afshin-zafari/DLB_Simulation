...
namespace dtsw{
  ...
  struct SpInfo{
    ...
    std::vector<std::pair<uint32_t,uint32_t> > index,org_index;
    int num_blocks_x,num_blocks_y,rb,cb,rmin,rmax,cmin,cmax;
    bool empty;
    ...
    ~SpInfo(){
      ...
      index.clear();
      org_index.clear();
      for(auto b: sp_blocks)
	delete b;      
    }
    SpInfo(){}
    SpInfo(int i, int j,bool ){cmin=rmin=10000000;rmax=cmax=0;}
    void report_data(){
