...
namespace dtsw{
  /*---------------------------------------------*/
  void AddTask::submit_next_level_tasks(){
    return;
    LOG_INFO(LOG_DTSW,"#NextLevel tasks early submit for %s.\n",getName().c_str());
  ...
  void AddTask::runKernel(){
    ...
    SGData &b = *B->sg_data;
    SGData &c = *C->sg_data;
    LOG_INFO(LOG_DTSW,"Add task :%s kernel called. sub tasks submitted:%d.\n",getName().c_str(),sub_tasks_submitted);
    if ( !sub_tasks_submitted){
      is_submitting = true;
  ...
  /*---------------------------------------------*/
  void RHSTask::submit_next_level_tasks(){
    return;
    sub_tasks_submitted = false;
    LOG_INFO(LOG_DTSW,"#NextLevel tasks early submit for %s.\n",getName().c_str());
    if  ( A->getHost() != me )
  ...
  void RHSTask::runKernel(){
    ...
    SGData &b = *B->sg_data;
    SGData &c = *C->sg_data;
    LOG_INFO(LOG_DTSW,"RHS kernel called.sub-tasks sbtd:%d\n",sub_tasks_submitted);
    if ( !sub_tasks_submitted){
  ...
  /*---------------------------------------------*/
  void DiffTask::submit_next_level_tasks(){
    return;
    LOG_INFO(LOG_DTSW,"#NextLevel tasks early submit for %s.\n",getName().c_str());
    sub_tasks_submitted = false;
    if  ( B->getHost() != me )
  ...
  void DiffTask::runKernel(){
    ...
    SGData &b = *B->sg_data;
    SGData &c = *C->sg_data;
    LOG_INFO(LOG_DTSW,"Diff task Kernel called. B is %s,GT-ver: rd:%s, wrt:%s, RT-ver: rd:%s, wrt:%s.. sub-tasks sbtd:%d.\n",
	     B->getName().c_str(),
	...
	for(int j=0;j<a.get_col_blocks(); j++){
	  if ( a(i,j).get_sp_info_ptr() == nullptr )
	    continue;
  ...
  void SGDiffTask::run(){
    ...
    assert(A.get_sp_info_ptr() != nullptr) ;
    for(int  j=0;j<Z; j++)
  ...
  /*---------------------------------------------*/
  void StepTask::submit_next_level_tasks(){
    return;
    LOG_INFO(LOG_DTSW,"#NextLevel tasks early submit for %s.\n",getName().c_str());
    sub_tasks_submitted = false;
    if  ( A->getHost() != me )
  ...
  void StepTask::runKernel(){
    ...
    SGData &d = *D->sg_data;
    SGData &e = *E->sg_data;
    LOG_INFO(LOG_DTSW,"StepTask kernel called. A:%s, sub-tasks sbtdd:%d.\n",
	     A->getName().c_str(),
	     sub_tasks_submitted);
    if ( !sub_tasks_submitted ){
      is_submitting = true;
  ...
  void SWTask::set_submitting(bool f){
    ...
    if  (!f){
      ...
      LOG_INFO(LOG_DTSW,"SyncTask created for %s.\n",getName().c_str());
    }
  }
}
