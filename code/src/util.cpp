...
namespace dtsw{
  ...
  void split(SpInfo  &M, int ny, int nx,int chunk_size){
    ...
    for(uint32_t i=0; i < M.data.size(); i++){
      ...
      block.rb = rbegin;
      block.cb = cbegin;
      block.org_index.push_back(make_pair(R,C));
      
      block.rmax = std::max(block.rmax,R);
      block.cmax = std::max(block.cmax,C);
      block.rmin = std::min(block.rmin,R);
      block.cmin = std::min(block.cmin,C);
  ...
  /*------------------------------------------*/
  SpInfo *split_var_size(SpInfo *M){
    int r1,r2,c1,c2;
    SpInfo *sp_ret=new SpInfo;
    assert( M);
    assert( M->num_blocks_y == M->num_blocks_x );
    //#define ASSERT(a,b) assert(((b)*B+(a)) < M->sp_blocks.size())
#define LOWER(a,b) ( (a)>0 and (b)>0 )
#define UPPER(a,b) ( (a)<B and (b)<B )
#define ASSERT(a,b) if (LOWER(a,b) and UPPER(a,b) ) {	\
	assert(((b)*B+(a)) < M->sp_blocks.size());	\
      }
#define BLK(a,b) (*M->sp_blocks[(b)*B+(a)])
    int B = M->num_blocks_y;
    int N = 3*B- 2;
    sp_ret->num_blocks_x = sp_ret->num_blocks_y=N;
    for(int ii=0;ii<N;ii++){
      for(int jj=0;jj<N;jj++){
	SpInfo  *spm = new SpInfo(ii,jj,true);
	sp_ret->sp_blocks.push_back(spm);
      }
    }
    int L = B-1;
    for (int i=0;i<B;i++){
      for(int j=0;j<B;j++){
	LOG_INFO(LOG_DTSW,"BLK(%d,%d) of %d.\n",i,j,M->sp_blocks.size() );
	ASSERT(i,j);
	SpInfo &sp=BLK(i,j);
	if(sp.data.size() ==0)
	  continue;
	ASSERT(i-1,j); ASSERT(i+1,j); ASSERT(i,j-1); ASSERT(i,j+1);
	
	c1 = (i == 0)?0            :BLK(i-1,j  ).cmax; //  up
	c2 = (i == L)?BLK(i,j).cmax:BLK(i+1,j  ).cmin; //  down
	r1 = (j == 0)?0            :BLK(i  ,j-1).rmax; //  left
	r2 = (j == L)?BLK(i,j).rmax:BLK(i  ,j+1).rmin; //  right
	for ( int k=0;k<sp.data.size(); k++){
	  int R=sp.org_index[k].first  ;
	  int C=sp.org_index[k].second ;
	  int br,bc,ri,ci;
	  if ( i==j){
	    br = i*3+(R<r1)?0:(R>r2?2:1)-((r1==0)?1:0);
	    bc = j*3+(C<c1)?0:(C>c2?2:1)-((c1==0)?1:0);
	    ri = R - (R<r1)?0:(R>r2?r2:r1);
	    ci = C - (C<c1)?0:(C>c2?c2:c1);
	  }
	  else{
	    if(i<j){br=i*3+2-(i==0);bc=j*3-1;}
	    if(j<i){bc=j*3+2-(j==0);br=i*3-1;}
	    ri=sp.index[k].first;
	    ci=sp.index[k].second;	    
	  }
	  if ((bc*N+br) >= sp_ret->sp_blocks.size()){
	    LOG_INFO(LOG_DTSW,"bc:%d, N:%d, br:%d, size:%d.\n",bc,N,br,sp_ret->sp_blocks.size());
	    assert(false);
	  }
	  SpInfo &sp_new = *sp_ret->sp_blocks[bc*N+br];
	  sp_new.data.push_back(sp.data[k]);
	  sp_new.index.push_back(make_pair(ri,ci));
	  sp_new.rmax = std::max(sp_new.rmax,R);
	  sp_new.cmax = std::max(sp_new.cmax,C);
	  sp_new.rmin = std::min(sp_new.rmin,R);
	  sp_new.cmin = std::min(sp_new.cmin,C);	  
	}
      }
    }
    //delete M;
  ...
  /*------------------------------------------*/
  void report_sp_info(SpInfo &M){
    for (int i=0;i<M.num_blocks_y;i++){
      for(int j=0;j<M.num_blocks_x;j++){
	SpInfo &sp=*M.sp_blocks[j*M.num_blocks_y+i];
	fprintf(stdout,"##Block at (%d,%d) Row span:%d-%d Col span:%d-%d.\n",
		i,j,sp.rmin,sp.rmax, sp.cmin , sp.cmax);
      }
    }
  }
  /*------------------------------------------*/
