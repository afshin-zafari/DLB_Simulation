...
namespace dtsw{
  ...
  void prepare_D(){
    ...
    int B=Parameters.partition_level[1].blocks_per_row;
    
    split(sp_info,B,B,chunk_size_L1);
#if VAR_BLOCK_SIZE
    report_sp_info(sp_info);
    SpInfo *temp = split_var_size(&sp_info);
    report_sp_info(*temp);
    spD = split_var_size(&sp_info);
    report_sp_info(sp_info);
    // TODO
#else
  ...
  void prepare_H(){
#if VAR_BLOCK_SIZE
    int nby =  3*Parameters.partition_level[1].Mb-2;
#else
    int nby  = Parameters.partition_level[1].Mb;
#endif
    int nby2 = Parameters.partition_level[2].blocks_per_row;
